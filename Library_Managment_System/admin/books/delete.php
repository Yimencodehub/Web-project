...
<body>
    ...
    <div class="main-wrapper">
        ...
        <div class="content">
            <div class="page-header"><h1 class="page-title">Delete Book</h1></div>
            <div class="card">
            <?php
            $id = $_GET['id'] ?? 0;
            try {
                ...
                if($check->fetchColumn() > 0) {
                    echo "<div class='alert alert-danger'>Cannot delete book: It has active issues.</div>";
                } else {
                    $del = $pdo->prepare("DELETE FROM books WHERE id=?");
                    if($del->execute([$id])) echo "<div class='alert alert-success'>Book deleted successfully. <a href='index.php'>Go back</a></div>";
                }
            } catch(Exception $e) { echo "<div class='alert alert-danger'>Error deleting book.</div>"; }
            ?></div>
        </div>
    </div>
</body>
