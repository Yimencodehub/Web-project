...
<body>
    ...
    <div class="main-wrapper">
        ...
        <div class="content">
            <div class="page-header"><h1 class="page-title">Edit Book</h1><a href="index.php" class="btn btn-primary">Back</a></div>
            <?php
            $id = (int)($_GET['id'] ?? 0);
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                try {
                    $stmt = $pdo->prepare("UPDATE books SET title=?, author=?, isbn=?, category_id=?, total_copies=?, description=?, shelf_location=?, publisher=?, year=? WHERE id=?");
                    $yearVal = !empty($_POST['publication_year']) ? (int)$_POST['publication_year'] : null;
                    if($stmt->execute([$_POST['title'], $_POST['author'], $_POST['isbn'], $_POST['category_id'], $_POST['total_copies'], $_POST['description'], $_POST['shelf_location'], $_POST['publisher'], $yearVal, $id])) {
                        echo "<div class='alert alert-success'>✅ Book updated successfully!</div>";
                    }
                } catch(Exception $e) { 
                    error_log($e->getMessage());
                    echo "<div class='alert alert-danger'>Error updating book: " . htmlspecialchars($e->getMessage()) . "</div>"; 
                }
            }
            $book = ['title'=>'', 'author'=>'', 'isbn'=>'', 'category_id'=>'', 'total_copies'=>1, 'description'=>'', 'shelf_location'=>'', 'publisher'=>'', 'year'=>''];
            try {
            ...
            <div class="card" style="max-width: 600px;">
                <form method="POST">
                    <div class="form-group"><label>Title *</label><input type="text" name="title" class="form-control" required value="<?=htmlspecialchars($book['title'])?>"></div>
                    <div class="form-group"><label>Author *</label><input type="text" name="author" class="form-control" required value="<?=htmlspecialchars($book['author'])?>"></div>
                    <div class="form-group"><label>ISBN</label><input type="text" name="isbn" class="form-control" value="<?=htmlspecialchars($book['isbn']??'')?>"></div>
                    ...
                    <div class="form-group"><label>Publisher</label><input type="text" name="publisher" class="form-control" value="<?=htmlspecialchars($book['publisher']??'')?>"></div>
                    <div class="form-group"><label>Publication Year</label><input type="number" name="publication_year" class="form-control" value="<?=htmlspecialchars($book['year']??'')?>"></div>
                    <div class="form-group"><label>Shelf Location</label><input type="text" name="shelf_location" class="form-control" value="<?=htmlspecialchars($book['shelf_location']??'')?>"></div>
                    <div class="form-group"><label>Total Copies *</label><input type="number" name="total_copies" class="form-control" required min="1" value="<?=htmlspecialchars($book['total_copies'])?>"></div>
                    <div class="form-group"><label>Description</label><textarea name="description" class="form-control" rows="4"><?=htmlspecialchars($book['description']??'')?></textarea></div>
