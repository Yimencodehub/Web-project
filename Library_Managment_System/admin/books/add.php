...
<body>
    ...
    <div class="main-wrapper">
        ...
        <div class="content">
            <div class="page-header"><h1 class="page-title">Add New Book</h1><a href="index.php" class="btn btn-primary">Back to Books</a></div>
            <?php
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                try {
                    $stmt = $pdo->prepare("INSERT INTO books (title, author, isbn, category_id, total_copies, available_copies, description, shelf_location, publisher, year) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                    $yearVal = !empty($_POST['publication_year']) ? (int)$_POST['publication_year'] : null;
                    if($stmt->execute([$_POST['title'], $_POST['author'], $_POST['isbn'], $_POST['category_id'], $_POST['total_copies'], $_POST['total_copies'], $_POST['description'], $_POST['shelf_location'], $_POST['publisher'], $yearVal])) {
                        echo "<div class='alert alert-success'>✅ Book added successfully!</div>";
                    }
                } catch(Exception $e) { 
                    error_log($e->getMessage());
                    echo "<div class='alert alert-danger'>Error adding book: " . htmlspecialchars($e->getMessage()) . "</div>"; 
                }
            }
            ?>
            <div class="card" style="max-width: 600px;">
                <form method="POST">
                    ...
                    <div class="form-group"><label>ISBN</label><input type="text" name="isbn" class="form-control"></div>
                    <div class="form-group"><label>Category *</label>
                        <select name="category_id" class="form-control" required><option value="">Select Category</option>
                            <?php
                            try { $cats = $pdo->query("SELECT id, name FROM categories")->fetchAll(); foreach($cats as $c) echo "<option value='{$c['id']}'>".htmlspecialchars($c['name'])."</option>"; } catch(Exception $e) {}
                            ?>
                        </select>
                    </div>
                    <div class="form-group"><label>Publisher</label><input type="text" name="publisher" class="form-control"></div>
                    <div class="form-group"><label>Publication Year</label><input type="number" name="publication_year" class="form-control"></div>
                    <div class="form-group"><label>Shelf Location</label><input type="text" name="shelf_location" class="form-control"></div>
                    <div class="form-group"><label>Total Copies *</label><input type="number" name="total_copies" class="form-control" required min="1" value="1"></div>
