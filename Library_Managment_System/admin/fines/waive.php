...
<body>
    ...
    <div class="main-wrapper">
        ...
        <div class="content">
            <div class="page-header"><h1 class="page-title">Waive Fine</h1><a href="index.php" class="btn btn-primary">Back</a></div>
            <div class="card">
            <?php
            $id = $_GET['id'] ?? 0;
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                try {
                    $pdo->prepare("UPDATE fines SET status='waived' WHERE id=?")->execute([$id]);
                    echo "<div class='alert alert-success'>Fine waived successfully! <a href='index.php'>Go Back</a></div>";
                } catch(Exception $e) { echo "<div class='alert alert-danger'>Error.</div>"; }
            } else {
                echo "<form method='POST'>
                    <div class='form-group'><label>Reason for waiving</label><textarea name='reason' class='form-control' required></textarea></div>
                    <button class='btn btn-danger'>Confirm Waive</button></form>";
            }
            ?>
            </div>
        </div>
