...
<body>
    ...
    <div class="main-wrapper">
        ...
        <div class="content">
            <div class="page-header"><h1 class="page-title">Manage Members</h1><a href="add.php" class="btn btn-primary">Add Member</a></div>
            <div class="card">
                <form class="search-bar" method="GET">
                    <input type="text" name="q" class="form-control" placeholder="Search name, email, ID..." value="<?=htmlspecialchars($_GET['q']??'')?>">
                    <button type="submit" class="btn btn-primary">Search</button>
                </form>
                <table>
                    <thead><tr><th>Member ID</th><th>Name</th><th>Email</th><th>Phone</th><th>Type</th><th>Status</th><th>Actions</th></tr></thead>
                    <tbody>
                        <?php
                        try {
                            $q = $_GET['q'] ?? '';
                            // members table has no full_name/email/phone — those live in users
                            $stmt = $pdo->prepare("
                                SELECT m.id, m.member_id, m.membership_type, m.status,
                                       u.full_name, u.email, u.phone
                                FROM members m
                                JOIN users u ON m.user_id = u.id
                                WHERE (u.full_name LIKE :q OR u.email LIKE :q OR m.member_id LIKE :q)
                                ORDER BY m.member_id ASC
                            ");
                            $stmt->execute(['q' => "%$q%"]);
                            while ($row = $stmt->fetch()) {
                                $badge = $row['status'] === 'active' ? 'badge-success' : 'badge-danger';
                                echo "<tr>
                                    ...
                                    <td>
                                        ...
                                        <a href='delete.php?id={$row['id']}' class='btn btn-sm btn-danger' onclick=\"return confirm('Deactivate this member?')\">Deactivate</a>
                                    </td>
                                </tr>";
                            }
                        } catch (Exception $e) {
                            error_log($e->getMessage());
                            echo '<tr><td colspan="7" style="color:#f87171">Error loading members.</td></tr>';
                        }
                        ?>
                    </tbody>
                </table>
