...
<body>
    ...
    <div class="main-wrapper">
        ...
        <div class="content">
            <div class="page-header"><h1 class="page-title">Edit Member</h1><a href="index.php" class="btn btn-primary">Back</a></div>
            <?php
            $id = $_GET['id'] ?? 0;
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                try {
                    $stmt = $pdo->prepare("UPDATE members SET full_name=?, email=?, phone=?, address=?, membership_type=?, expiry_date=?, status=? WHERE id=?");
                    if($stmt->execute([$_POST['full_name'], $_POST['email'], $_POST['phone'], $_POST['address'], $_POST['membership_type'], $_POST['expiry_date'], $_POST['status'], $id])) echo "<div class='alert alert-success'>Updated!</div>";
                } catch(Exception $e) { echo "<div class='alert alert-danger'>Error.</div>"; }
            }
            $mem = ['full_name'=>'', 'email'=>'', 'phone'=>'', 'address'=>'', 'membership_type'=>'regular', 'expiry_date'=>'', 'status'=>'active'];
            try { $stmt = $pdo->prepare("SELECT * FROM members WHERE id=?"); $stmt->execute([$id]); $m = $stmt->fetch(); if($m) $mem = $m; } catch(Exception $e) {}
            ?>
            <div class="card" style="max-width: 600px;">
                <form method="POST">
                    <div class="form-group"><label>Full Name *</label><input type="text" name="full_name" class="form-control" required value="<?=htmlspecialchars($mem['full_name'])?>"></div>
                    <div class="form-group"><label>Email *</label><input type="email" name="email" class="form-control" required value="<?=htmlspecialchars($mem['email'])?>"></div>
                    <div class="form-group"><label>Phone</label><input type="text" name="phone" class="form-control" value="<?=htmlspecialchars($mem['phone']??'')?>"></div>
                    <div class="form-group"><label>Address</label><textarea name="address" class="form-control"><?=htmlspecialchars($mem['address']??'')?></textarea></div>
                    <div class="form-group"><label>Membership Type</label>
                        <select name="membership_type" class="form-control">
                            <option value="regular" <?=$mem['membership_type']=='regular'?'selected':''?>>Regular</option>
                            <option value="premium" <?=$mem['membership_type']=='premium'?'selected':''?>>Premium</option>
                        </select>
                    </div>
                    <div class="form-group"><label>Status</label>
                        <select name="status" class="form-control">
                            <option value="active" <?=$mem['status']=='active'?'selected':''?>>Active</option>
                            <option value="inactive" <?=$mem['status']=='inactive'?'selected':''?>>Inactive</option>
                        </select>
                    </div>
                    <div class="form-group"><label>Expiry Date</label><input type="date" name="expiry_date" class="form-control" value="<?=htmlspecialchars($mem['expiry_date']??'')?>"></div>
