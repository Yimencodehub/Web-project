...
<body>
    ...
    <div class="main-wrapper">
        ...
        <div class="content">
            <div class="page-header"><h1 class="page-title">Add Member</h1><a href="index.php" class="btn btn-primary">Back</a></div>
            <?php
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                try {
                    ...
                    $member_id = "LIB" . str_pad($user_id, 4, "0", STR_PAD_LEFT);
                    $stmt2 = $pdo->prepare("INSERT INTO members (user_id, member_id, full_name, email, phone, address, membership_type, expiry_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
                    $stmt2->execute([$user_id, $member_id, $_POST['full_name'], $_POST['email'], $_POST['phone'], $_POST['address'], $_POST['membership_type'], $_POST['expiry_date']]);
                    $pdo->commit();
                    echo "<div class='alert alert-success'>Member added successfully! ID: $member_id</div>";
                } catch(Exception $e) { $pdo->rollBack(); echo "<div class='alert alert-danger'>Error adding member.</div>"; }
            }
            ?>
            <div class="card" style="max-width: 600px;">
                <form method="POST">
                    ...
                    <div class="form-group"><label>Membership Type</label>
                        <select name="membership_type" class="form-control"><option value="regular">Regular</option><option value="premium">Premium</option></select>
                    </div>
                    <div class="form-group"><label>Expiry Date</label><input type="date" name="expiry_date" class="form-control" value="<?=date('Y-m-d', strtotime('+1 year'))?>"></div>
