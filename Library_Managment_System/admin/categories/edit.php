...
<body>
    ...
    <div class="main-wrapper">
        ...
        <div class="content">
            <div class="page-header"><h1 class="page-title">Edit Category</h1><a href="index.php" class="btn btn-primary">Back</a></div>
            <?php
            $id = $_GET['id'] ?? 0;
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                try {
                    $stmt = $pdo->prepare("UPDATE categories SET name=?, description=? WHERE id=?");
                    if($stmt->execute([$_POST['name'], $_POST['description'], $id])) echo "<div class='alert alert-success'>Updated!</div>";
                } catch(Exception $e) { echo "<div class='alert alert-danger'>Error.</div>"; }
            }
            $cat = ['name'=>'', 'description'=>''];
            try { $stmt = $pdo->prepare("SELECT * FROM categories WHERE id=?"); $stmt->execute([$id]); $c = $stmt->fetch(); if($c) $cat = $c; } catch(Exception $e) {}
            ?>
            <div class="card" style="max-width: 600px;">
                <form method="POST">
                    <div class="form-group"><label>Name *</label><input type="text" name="name" class="form-control" required value="<?=htmlspecialchars($cat['name'])?>"></div>
                    <div class="form-group"><label>Description</label><textarea name="description" class="form-control" rows="4"><?=htmlspecialchars($cat['description']??'')?></textarea></div>
