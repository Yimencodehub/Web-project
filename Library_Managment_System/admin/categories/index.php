...
<body>
    ...
    <div class="main-wrapper">
        ...
        <div class="content">
            <div class="page-header"><h1 class="page-title">Manage Categories</h1><a href="add.php" class="btn btn-primary">Add Category</a></div>
            <div class="card">
                <table>
                    <thead><tr><th>ID</th><th>Name</th><th>Description</th><th>Book Count</th><th>Actions</th></tr></thead>
                    <tbody>
                        <?php
                        try {
                            $stmt = $pdo->query("SELECT c.*, (SELECT COUNT(*) FROM books WHERE category_id=c.id) as book_count FROM categories c");
                            while($row = $stmt->fetch()) {
                                echo "<tr>
                                    <td>{$row['id']}</td><td>".htmlspecialchars($row['name'])."</td><td>".htmlspecialchars($row['description']??'')."</td>
