...
<body>
    ...
    <div class="main-wrapper">
        ...
        <div class="content">
            <div class="page-header"><h1 class="page-title">Manage Staff Users</h1><a href="add.php" class="btn btn-primary">Add User</a></div>
            <div class="card">
                <table>
                    ...
                    <tbody>
                        <?php
                        try {
                            $stmt = $pdo->query("SELECT * FROM users WHERE role IN ('admin', 'staff', 'superadmin')");
                            while($row = $stmt->fetch()) {
                                echo "<tr>
                                    <td>{$row['id']}</td><td>".htmlspecialchars($row['username'])."</td><td>".htmlspecialchars($row['email']??'N/A')."</td>
